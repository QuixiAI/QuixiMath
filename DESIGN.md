...
- **Graph Interpretation (Bar/Line/Pictograph/Double Bar):** Graph data recording (`GRAPH_DATA`; one line per series for a double-bar chart), value reading (`GRAPH_READ`), raw-category construction (`SORT`, `COUNT`), comparisons (`CMP`), min/max identification (`GRAPH_MIN`, `GRAPH_MAX`), change tracking (`GRAPH_CHANGE`, `GRAPH_MAX_CHANGE`), pictograph key (`PICTO_KEY`) and symbol counting (`PICTO_COUNT`), arithmetic steps reused (`A`, `S`, `M`), exact checks (`CHECK`), final `Z`.
