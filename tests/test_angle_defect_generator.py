...
PROBLEM_RE = re.compile(
    r"A hyperbolic triangle with curvature radius (\d+) has angles "
    r"(\d+) deg, (\d+) deg, and (\d+) deg\. Use the angle defect "
    r"formula to find its exact area in terms of pi\."
)
...
def expected_flow(example):
    ...
    steps = [
        make_step("ANGLE_DEFECT_SETUP", f"R={parts['radius']}",
                  f"angles={parts['a']},{parts['b']},{parts['c']}"),
        make_step("THEOREM", "hyperbolic angle defect",
                  "area = (180 deg - (A+B+C))/180 * pi * R^2"),
        make_step("A", parts["a"], parts["b"], first_sum),
        make_step("A", first_sum, parts["c"], angle_sum),
        make_step("S", 180, angle_sum, defect_deg),
        make_step("D", defect_deg, 180, defect_turn),
        make_step("E", parts["radius"], 2, radius_sq),
        make_step("M", defect_turn, radius_sq, area_coeff),
        make_step("Z", answer),
    ]
    return steps, answer


class TestAngleDefectGenerator(unittest.TestCase):
    def setUp(self):
        random.seed(42)
        self.gen = AngleDefectGenerator()
    ...
    def test_output_contract(self):
        ...
        self.assertEqual(result["operation"], "hyperbolic_angle_defect_area")
    ...
    def test_area_is_angle_defect_times_radius_squared(self):
        for _ in range(300):
            result = self.gen.generate()
            parts = parse_problem(result["problem"])
            defect = Fraction(180 - parts["a"] - parts["b"] - parts["c"],
                              180)
            answer = f"area = {pi_text(defect * parts['radius'] ** 2)}"
