...
- [x] **PolynomialInequalityGenerator** (high) — quadratic/polynomial/rational
...
- [x] **MasterTheoremGenerator** (college) — recurrence complexity:
...
- [x] **TelescopingGenerator** (high/college) — telescoping sums and products.
...
- [x] **TwoSampleTestGenerator** (high/college) — two-sample t and
  ...
  and decision.
- [x] **EquilibriumICEGenerator** (high, chemistry) — ICE tables. Variants:
...
- [x] **SeparablePDEGenerator** (graduate) — heat equation
...
- [x] **InductionVerifyGenerator** (high/college) — verify P(1) numerically,
  ...
  identity confirmed).
- [x] **QRDecompositionGenerator** (college) — QR via Gram-Schmidt; extend the
...
- [x] **CholeskyGenerator** (college) — build A = L L^T from integer L (same
  trick PositiveDefiniteGenerator already uses), then recover L step by step.
  Check step: L L^T = A.
- [x] **CountingClassicsGenerator** (college) — pigeonhole (minimum n to
...
- [x] Sweep higher-tier generators for single-template problem statements and
...
- [x] Audit per-type capacity: probe every generator for distinct-problem
  count (script exists in the review history: sample ~2000, count distinct
  problem texts). Flag anything under ~1000.
- [x] Widen the known flagged generators — same treatment QuantumGate (12 → 170+,
  ...
  A full-repo probe now exists and was run; it also flags many pre-existing
  finite-space generators outside this known follow-up list.
